    ...
    <article className="border-[3px] border-black bg-white p-4 shadow-[6px_6px_0_0_rgba(0,0,0,0.08)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 space-y-2">
          <p className="font-serif text-lg font-black leading-tight">{wp.displayName}</p>
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-black/55">
            NPWPD {wp.npwpd || "-"}
          </p>
        </div>
        <div className="flex flex-col items-end gap-2">
          <Badge className="rounded-none border-[2px] border-black bg-[#FFFF00] font-mono text-[10px] text-black">
            {compactStatusLabel(wp.statusAktif)}
          </Badge>
          <Badge className="rounded-none border-[2px] border-black bg-[#FF6B00] font-mono text-[10px] text-white">
      ...
      <div className="mt-4 flex gap-2">
        {canMutate ? (
          <Button
            type="button"
            variant="outline"
            className="flex-1 rounded-none border-[2px] border-black font-mono text-[11px] font-bold"
            onClick={() => onEdit(wp)}
          >
        ...
        {canMutate ? (
          <Button
            type="button"
            variant="outline"
            className="rounded-none border-[2px] border-red-600 px-3 font-mono text-[11px] font-bold text-red-600"
