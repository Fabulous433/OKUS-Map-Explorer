...
export default function BackofficeWajibPajak() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: WpFormValues) => (await apiRequest("POST", "/api/wajib-pajak", toPayload(data, "create"))).json(),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/wajib-pajak"] }); setOpenAdd(false); addForm.reset(defaults()); toast({ title: "Berhasil", description: "Wajib Pajak berhasil ditambahkan" }); },
    onError: (err: Error) => toast({ title: "Gagal", description: err.message, variant: "destructive" }),
  });

  const updateMutation = useMutation({
    mutationFn: async (p: { id: number; data: WpFormValues }) => (await apiRequest("PATCH", `/api/wajib-pajak/${p.id}`, toPayload(p.data, "edit"))).json(),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/wajib-pajak"] }); setEdit(null); toast({ title: "Berhasil", description: "Wajib Pajak berhasil diperbarui" }); },
    onError: (err: Error) => toast({ title: "Gagal", description: err.message, variant: "destructive" }),
  });
  ...
  const openEdit = (wp: WajibPajakWithBadanUsaha) => { setEdit(wp); editForm.reset(defaults(wp)); };
    ...
    <BackofficeLayout>
      <div className="p-6" data-testid="backoffice-wp-page">
        ...
        <div className="mb-6 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Button className="rounded-none border-[3px] border-black bg-[#FFFF00] text-black font-mono font-bold" onClick={() => { addForm.reset(defaults()); setOpenAdd(true); }}><Plus className="w-4 h-4 mr-2" />TAMBAH WP</Button>
          </div>
        </div>
              ...
              <TableBody>
                {filtered.map((wp) => (
                  <TableRow key={wp.id} className="border-b-[2px] border-black">
                    ...
                    <TableCell className="text-right"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" className="rounded-none" onClick={() => openEdit(wp)}><Pencil className="w-4 h-4" /></Button><Button size="icon" variant="ghost" className="rounded-none text-red-600" onClick={() => deleteMutation.mutate(wp.id)}><Trash2 className="w-4 h-4" /></Button></div></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Dialog open={openAdd} onOpenChange={setOpenAdd}>
        <DialogContent className="rounded-none border-[4px] border-black max-w-2xl bg-white p-0 max-h-[90vh] overflow-y-auto">
          ...
          <Form {...addForm}><form onSubmit={addForm.handleSubmit((data) => createMutation.mutate(data))} className="p-4 space-y-4"><WpForm form={addForm} mode="create" /><Button type="submit" className="w-full rounded-none border-[3px] border-black bg-[#FF6B00] text-white font-mono font-bold">{createMutation.isPending ? "MENYIMPAN..." : "SIMPAN"}</Button></form></Form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!edit} onOpenChange={(open) => !open && setEdit(null)}>
        <DialogContent className="rounded-none border-[4px] border-black max-w-2xl bg-white p-0 max-h-[90vh] overflow-y-auto">
          ...
          <Form {...editForm}><form onSubmit={editForm.handleSubmit((data) => edit && updateMutation.mutate({ id: edit.id, data }))} className="p-4 space-y-4"><WpForm form={editForm} mode="edit" /><Button type="submit" className="w-full rounded-none border-[3px] border-black bg-blue-600 text-white font-mono font-bold">{updateMutation.isPending ? "MEMPERBARUI..." : "PERBARUI"}</Button></form></Form>
        </DialogContent>
      </Dialog>
    </BackofficeLayout>
  );
