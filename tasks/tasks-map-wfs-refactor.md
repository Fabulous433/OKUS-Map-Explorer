...
- Update 2026-03-19 (mobile compact shell pass): evidence yang sama sekarang juga memuat addendum mobile untuk header title-only `18px` dan chip filter desa ringkas `Semua/WLT`.
...
- Update 2026-03-19 (production parity fix): mismatch staging vs localhost berhasil di-root-cause ke route `/api/region-boundaries/active/*` yang sehat di `tsx` dev tetapi gagal di `dist/index.cjs`, sehingga staging hanya menampilkan fallback center/zoom tanpa polygon wilayah. Fix sudah masuk di `server/region-boundaries.ts` dan regression baru mem-boot server production bundle sebelum assert route boundary `200`. Staging perlu redeploy commit ini sebelum smoke Task `7.0` dilanjutkan.
